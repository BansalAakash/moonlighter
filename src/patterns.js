...
const CHROME_LINE = [
  ...
  // The context-limit hint row (see CONTEXT_LIMIT_RENDER). It is a fixed notification row
  // the TUI parks next to the input box, so it is furniture by the same rule as the mode
  // footer — and it MUST be listed here, because it is the one render that would otherwise
  // sit BELOW the box as a non-chrome line: the trailing-chrome scan would stop dead on it
  // and hand every other detector a 12-line window made almost entirely of box rules and
  // footer rows instead of transcript. Stripping it costs those detectors nothing (it names
  // no limit and no API error), and contextLimitMatch reads the RAW capture, so this entry
  // does not hide the row from its own detector.
  /^\s*Context limit reached\s*·/,
];
...
// --- Context-limit hint row ---
// When the conversation no longer fits the model's window, Claude Code stops accepting the
// turn and parks a fixed notification row next to the input box. Verified against the
// v2.1.263 binary, which builds it as:
//   <Text color="error">{"Context limit reached · "}{tz}{Nh}{zS ? ` · ${zS}` : ""}</Text>
//     tz = DISABLE_COMPACT ? "/clear to continue" : "/compact or /clear to continue"
//     Nh = " · auto-compact is off · /config to turn it on"   (only when it is off)
//     zS = an optional trailing warning
// so the live row reads, in full:
//   Context limit reached · /compact or /clear to continue[ · auto-compact is off · …][ · …]
//
// The session is STUCK here: nothing resumes it until someone compacts or clears. That is
// the gap this closes — but the remedy is the only semi-destructive action in this package
// (compaction summarises history irreversibly), so the render is matched far more strictly
// than the phrase-anchored families above:
//
//   1. THE WHOLE LINE, not a phrase inside it. Ink renders this row alone, so a real one IS
//      the pattern end to end (bar the documented "· …" suffixes). Prose discussing the
//      limit — including this package's own README and any session that talks about it —
//      carries the words mid-sentence and fails the anchor. A phrase match would have been
//      self-triggering: a monitored session merely READING about this feature would compact
//      itself.
//   2. NO GLYPH. A leading ⏺/● is Claude's message glyph and ❯/> is the user's input row;
//      neither prefixes the notification row, and both are exactly how a QUOTATION of it
//      arrives. The pattern starts at optional whitespace and then the words.
//   3. /compact MUST BE OFFERED. Under DISABLE_COMPACT the row only offers "/clear to
//      continue", and clearing throws the conversation away — never something to automate.
//      Requiring the /compact affordance means that configuration is detected-but-ignored,
//      which is the correct refusal rather than a missed case.
//   4. THE LIVE REGION, the same discipline the /usage-credits companion uses: everything
//      below the row must be chrome. A row scrolled up into history has real content under
//      it and is no longer the session's state.
// Idle-only and foreground-gated at the call site, and bounded by maxRetries.
const CONTEXT_LIMIT_RENDER = /^\s*Context limit reached\s*·\s*\/compact or \/clear to continue(?:\s*·.*)?$/i;
// The same row when compaction is unavailable (DISABLE_COMPACT). Detected so the monitor can
// say why it is standing down instead of looking simply blind.
const CONTEXT_LIMIT_CLEAR_ONLY = /^\s*Context limit reached\s*·\s*\/clear to continue(?:\s*·.*)?$/i;

// { line, actionable } for a live context-limit row, else null. actionable:false marks the
// /clear-only variant — detected, never acted on.
export function contextLimitMatch(text) {
  const all = stripAnsi(text).split('\n');
  const mask = toolEchoMask(all);
  for (let i = all.length - 1; i >= 0; i--) {
    if (mask[i]) continue;
    const actionable = CONTEXT_LIMIT_RENDER.test(all[i]);
    if (!actionable && !CONTEXT_LIMIT_CLEAR_ONLY.test(all[i])) continue;
    // Live region: only the input box / footer may follow it. Note CHROME_LINE carries this
    // very row, so a SECOND (stale) copy below would also read as chrome — harmless: the
    // scan runs bottom-up, so the lowest one is the one we are standing on.
    if (!all.slice(i + 1).every(isChromeLine)) return null;
    return { line: all[i].trim().slice(0, 200), actionable };
  }
  return null;
}

export function detectContextLimit(text) {
  const m = contextLimitMatch(text);
  return m !== null && m.actionable;
}
