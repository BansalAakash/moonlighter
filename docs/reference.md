...
The full documentation inherited from upstream
[cheapestinference/claude-auto-retry](https://github.com/cheapestinference/claude-auto-retry)
v0.7.3, kept for the detail the main [README](../README.md) deliberately leaves out:
every config key, the detection rules, overload backoff, environment forwarding and the
tmux/launcher internals.

Two things below are upstream's and do not apply to this fork: `npm i -g claude-auto-retry`
installs upstream, not this (use [`install.sh`](../install.sh)), and `retryMessage` is no
longer what a usage-limit retry sends here — see `usageLimitMessage` in the main README.
