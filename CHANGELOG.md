...
- **Context-limit compaction (`contextLimit`).** When the window fills, Claude Code parks a
  `Context limit reached · /compact or /clear to continue` row next to the input box and
  accepts nothing further — an unattended session stops there for good. Observed overnight:
  the usage limit reset, the continuation message went in, and the session answered with that
  row and sat until morning. The monitor now compacts and resumes: send `/compact`, let the
  compaction turn run to completion, then re-send the continuation (by default the same
  `usageLimitMessage` the stuck turn could not process).

  Notes on the safety envelope, since compaction is the only irreversible thing this package
  does:
  - The render is matched as a WHOLE LINE with no leading message/prompt glyph, and only when
    nothing but chrome sits below it. Verified against a live pane both ways: the phrase typed
    into the input box, and the same phrase emitted by Claude itself (which renders as
    `⏺ Context limit reached · …`), are both refused; the row parked by the input box matches.
    A phrase-level match would have been self-triggering — a session merely reading about this
    feature would compact itself.
  - Under `DISABLE_COMPACT` the row only offers `/clear`. That variant is detected and
    deliberately NOT acted on: clearing discards the conversation.
  - Idle-only, foreground-gated, bounded by `maxRetries` (2), and outranked by a usage limit.
- **Tests.** `test/` restored from upstream v0.7.3 (the npm tarball ships none) plus
  `test/context-limit.test.js`, which runs against `test/fixture-idle-pane.txt` — a real
...
- **`loadConfig` now validates the no-config path too.** It returned the raw `DEFAULT_CONFIG`
  ...
  `usageLimitMessage`", and resolves inside `validate()` — so on a machine with no config file
  the monitor would have typed a literal `null` into the pane after compacting.
- **The context-limit row no longer squeezes the other detectors.** It is the one render that
  sits BELOW the input box as a non-chrome line, so the trailing-chrome scan stopped dead on
  it and handed `isRateLimited` a 12-line window made of box rules and footer instead of
  transcript — a live usage-limit banner a few rows up would have gone undetected for as long
  as the context row was on screen. It is now listed as chrome; `contextLimitMatch` reads the
  raw capture, so its own detector is unaffected.
